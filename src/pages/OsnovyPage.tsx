...
const topics: Topic[] = [
  ...
  },
  {
    ...
    icon: "FileCode",
    intro: "После установки Python на компьютере появляется программа IDLE — это специальный редактор, созданный для написания кода на Python. Думай о нём как о блокноте, но для программиста. Открой его через меню Пуск → IDLE (Python).",
    steps: [
      {
        label: "Что такое IDLE",
        text: "IDLE — это программа, которая идёт в комплекте с Python. В ней два режима работы: консоль (Shell) и файл. Именно путаница между ними вызывает большинство ошибок у новичков.",
      },
      {
        label: "Что такое Shell",
        text: "Shell (или консоль) — это первое окно, которое открывается в IDLE. Оно выглядит как чёрный или белый экран с символами >>>. Это означает «жду команду». Сюда можно вводить только одну строку за раз — нажал Enter, получил ответ. Для изучения одиночных команд — удобно. Для написания программы — не подходит.",
      },
      {
        label: "Что такое файл",
        text: "Файл — это как обычный текстовый документ, только с кодом. Здесь можно написать сразу 10, 50, 100 строк кода, а потом запустить всё разом. Именно в файл нужно вставлять все примеры из этого курса.",
      },
      {
        label: "Почему такое разделение",
        text: "Такой подход существует во всех языках программирования. Shell — для быстрых экспериментов (проверить одну команду). Файл — для написания настоящих программ. Это как черновик и чистовик.",
      },
      {
        label: "Ошибка в IDLE",
        text: "Если видишь «SyntaxError: multiple statements» — ты случайно вставил несколько строк в Shell. Это легко исправить: просто используй файл вместо консоли.",
      },
    ],
    blocks: [
      {
        label: "Как правильно запустить код — пошагово:",
        code: `1. Открой IDLE через меню Пуск
...
7. Результат появится в окне Shell`,
        comment: "Запомни: код пишем в файле, результат смотрим в Shell",
      },
    ],
  },
  {
