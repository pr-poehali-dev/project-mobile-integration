...
const topics: Topic[] = [
  ...
  },
  {
    ...
    icon: "FileCode",
    intro: "Прежде чем писать код — важно понять, куда именно его вводить. В IDLE есть два разных окна, и они работают по-разному.",
    steps: [
      { label: "Shell (консоль)", text: "Окно с >>> — это консоль. Сюда вводишь по одной строке и сразу видишь результат. Несколько строк сразу вставить нельзя." },
      { label: "Файл", text: "Чтобы писать настоящий код — открой файл: File → New File. Вставь весь код, затем нажми F5 (или Run → Run Module). Результат появится в Shell." },
      { label: "Ошибка в IDLE", text: "Если видишь «SyntaxError: multiple statements» — ты вставил несколько строк в консоль Shell. Реши это просто: используй файл (File → New File)." },
    ],
    blocks: [
      {
        label: "Правильный порядок действий:",
        code: `1. Открыть IDLE
2. File → New File  (открылось пустое окно)
3. Написать код в этом окне
4. Нажать F5
5. Результат появится в Shell`,
      },
    ],
  },
  {
